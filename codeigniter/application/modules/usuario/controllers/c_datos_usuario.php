...
class C_datos_usuario extends MX_Controller {
	...
	function processFiltro($offset = '') {		
		...
		$config['total_rows'] = $this->getCantPedidos($filtro_count);						
		$config['per_page'] = $limite;
		$config['uri_segment'] = 4;		
		$config['next_tag_open'] = '<li>';
		$config['next_tag_close'] = '</li>';
		...
		$config['cur_tag_open'] = '<li>';
		$config['cur_tag_close'] = '</li>';
		$data['oculto_url'] = form_hidden('oculto_url','processFiltro');
	...
	function getCantPedidos($param_pedidos = array()) {
		$p = new Pedido();
		if (empty($param_pedidos)) {
			return $p->getCantPedUsuario(2); //$this->id_usuario
		}else {
			return $p->getCantPedUsuario(2,$param_pedidos);
		}		
	}
	...
	function filtrarBusqueda($limit,$offset) {		
		...
		//si esta seleccionado solo fecha
			...
			if ($ped_fecha == '1') {
				...
				return $p->getPedidosUsuarios(2,$limit,$offset,'fechaPedido asc, horaPedido asc'); //$this->id_usuario
			}
		}
	}	
}
